...
export function useGeometry () {
  ...
  const updateBlockedMoveGeometryRef = useRef<any>(null)
  ...
  const setupOnView = useCallback((view: MapView) => {
    ...
    svm.on('update', async (ev: any) => {
      if (sketchModeRef.current === 'updating') {
        const prevGeometry = updateSketchGraphicRef.current?.geometry?.clone?.() || null
        ...
        if (
          ...
          ev.toolEventInfo &&
          String(ev.toolEventInfo.type).includes('move')
        ) {
          if (!updateBlockedMoveGeometryRef.current) {
            updateBlockedMoveGeometryRef.current = prevGeometry
          }
        ...
        if (ev.state === 'cancel' || ev.state === 'complete') {
          ...
          const savedGeom = updateBlockedMoveGeometryRef.current || (ev.graphics?.[0] ?? updateSketchGraphicRef.current)?.geometry
          updateBlockedMoveGeometryRef.current = null
