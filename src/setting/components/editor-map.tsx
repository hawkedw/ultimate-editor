...
function getEditableFeatureLayersFromView (jmv: JimuMapView | null): LayerMeta[] {
  ...
  const all = view?.map?.allLayers?.toArray?.() ?? []
  const fls = all.filter((l: any) => l?.type === 'feature' && !l?.isTable)
...
function mergeRule (meta: LayerMeta, existing: LayerRule | null, checked: boolean): LayerRule {
  const base: LayerRule = existing ? toPlain(existing) : { id: meta.key, url: meta.url, title: meta.title }
  if (checked) {
    return {
      ...base,
      id: meta.key,
...
export default function EditorMap (props: {
  ...
  React.useEffect(() => {
    ...
    if (next.length > 0) {
      ...
      if (!hasAnyEnabled) {
        ...
        onConfigChange((config as any).set('layers', Immutable(dedupeById([...others, ...newRules]))))
      }
    }
  ...
  const updateRulesBulk = (nextRules: LayerRule[]) => {
    onConfigChange((config as any).set('layers', Immutable(dedupeById(nextRules))))
  }
  ...
  const activeRule = activeLayer
    ? (findRule(rules, activeLayer) ?? { id: activeLayer.key, url: activeLayer.url, title: activeLayer.title })
    : null
