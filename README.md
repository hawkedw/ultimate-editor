...
config.ts / config.json           ← схема настроек виджета (allowCreate, allowAttrUpdate, …)
```
...
| `startGeometryEdit(item, onDone)` | Копирует графику в SVM, прячет оригинал через `layerView.filter`, запускает `update(reshape)` |
